    ...
    if ( file_exists( $uwb_config_file ) ) {
        ...
        if ( is_array( $uwb_config_data ) ) {
            if ( empty( $uwb_config_data['redis_enabled'] ) ) {
                ...
                if ( ! defined( 'WP_REDIS_PATH' ) && ! empty( $uwb_config_data['redis_socket'] ) ) {
                    define( 'WP_REDIS_PATH', $uwb_config_data['redis_socket'] );
                }
                if ( ! defined( 'WP_REDIS_HOST' ) && ! empty( $uwb_config_data['redis_host'] ) ) {
                    define( 'WP_REDIS_HOST', $uwb_config_data['redis_host'] );
                }
                if ( ! defined( 'WP_REDIS_PORT' ) && ! empty( $uwb_config_data['redis_port'] ) ) {
                    define( 'WP_REDIS_PORT', intval( $uwb_config_data['redis_port'] ) );
                }
                if ( ! defined( 'WP_REDIS_PASSWORD' ) && ! empty( $uwb_config_data['redis_password'] ) ) {
                    define( 'WP_REDIS_PASSWORD', $uwb_config_data['redis_password'] );
                }
                if ( ! defined( 'WP_REDIS_DATABASE' ) && isset( $uwb_config_data['redis_db'] ) ) {
...
class WP_Object_Cache {
    ...
    /**
     * The amount of microseconds (Î¼s) waited for Redis commands.
     *
     * @var float
     */
    public $cache_time = 0;
