...
define( 'UWB_VERSION', '1.19.1' );
define( 'UWB_PLUGIN_FILE', __FILE__ );
define( 'UWB_PLUGIN_DIR', plugin_dir_path( __FILE__ ) );


// 0. Autoload for Ultimate_WP_Booster\Dependencies (matthiasmullie/minify package)
spl_autoload_register( function( $class ) {
    $prefix = 'Ultimate_WP_Booster\\Dependencies\\';
    $len = strlen( $prefix );
    if ( strncmp( $prefix, $class, $len ) !== 0 ) {
        return;
    }
    $relative_class = substr( $class, $len );
    $file = UWB_PLUGIN_DIR . 'includes/Dependencies/' . str_replace( '\\', '/', $relative_class ) . '.php';
    if ( file_exists( $file ) ) {
        require_once $file;
    }
} );
...
// 1.5. Include compatibility layers (dynamically load all files in compatibility directory)
