		...
		if ( defined( 'WP_CONTENT_DIR' ) ) {
			...
			if ( file_exists( $uwb_config_file ) ) {
				...
				if ( is_array( $uwb_config_data ) ) {
					// Check if Memcached (2) is configured
					if ( isset( $uwb_config_data['redis_enabled'] ) && intval( $uwb_config_data['redis_enabled'] ) === 2 ) {
						$mc_host = ! empty( $uwb_config_data['redis_host'] ) ? $uwb_config_data['redis_host'] : '127.0.0.1';
						$mc_port = ! empty( $uwb_config_data['redis_port'] ) ? intval( $uwb_config_data['redis_port'] ) : 11211;
						if ( $mc_port === 6379 ) {
							$mc_port = 11211;
						}
						$memcached_servers = array( array( $mc_host, $mc_port ) );
					}
				}
			}
		}
